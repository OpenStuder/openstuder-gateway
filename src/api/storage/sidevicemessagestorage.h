...
class SIDeviceMessageStorage {
  public:
    ...
    QVector<TimestampedDeviceMessage> retrieveDeviceMessages(const QDateTime& from, const QDateTime& to = QDateTime::currentDateTime());

  private:
    virtual bool storeDeviceMessages_(const QVector<SIDeviceMessage>& messages, const QDateTime& timestamp) = 0;
    virtual QVector<TimestampedDeviceMessage> retrieveDeviceMessages_(const QDateTime& from, const QDateTime& to) = 0;
};
